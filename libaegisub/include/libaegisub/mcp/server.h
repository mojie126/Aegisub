...
/// @brief 主进程内嵌的 Model Context Protocol 服务器，Streamable HTTP 传输
/// @ingroup mcp
///
...
json::UnknownElement CallTool(std::string const& name,
                              json::Object const& arguments);
...
/// 启动 Streamable HTTP 上的 MCP JSON-RPC 主循环，阻塞至 server 停止，
/// 该函数会启动一个后台线程接受 TCP 连接，tool 调用通过
