...
namespace {
	...
	std::string GetString(Object const &obj, const std::string_view key) {
	...
	int64_t GetInt(Object const &obj, const std::string_view key, const int64_t def = 0) {
		const auto it = obj.find(key);
	...
	auto GetBool(Object const &obj, const std::string_view key, bool def = false) -> bool {
		const auto it = obj.find(key);
		if (it == obj.end()) return def;
		try {
	...
	[[maybe_unused]] auto GetDouble(Object const &obj, const std::string_view key, double def = 0.0) -> double {
		const auto it = obj.find(key);
		if (it == obj.end()) return def;
		try {
	...
	UnknownElement AssStyleToJson(AssStyle const &s) {
		...
		obj["angle"] = s.angle;
		obj["borderstyle"] = s.borderstyle;
		obj["outline_w"] = s.outline_w;
		obj["shadow_w"] = s.shadow_w;
		obj["alignment"] = s.alignment;
		obj["margin_l"] = s.Margin[0];
		obj["margin_r"] = s.Margin[1];
		obj["margin_v"] = s.Margin[2];
		obj["encoding"] = s.encoding;
		return obj;
	}
	...
	int ApplyStyleFields(AssStyle &s, Object const &fields) {
		...
		auto set_str = [&](const std::string_view key, std::string AssStyle::*field) {
			const auto it = fields.find(key);
			if (it == fields.end()) return;
			try {
				s.*field = static_cast<std::string const &>(it->second);
				++applied;
			} catch (...) {}
		};
		auto set_double = [&](const std::string_view key, double AssStyle::*field) {
			const auto it = fields.find(key);
			if (it == fields.end()) return;
			try {
				s.*field = static_cast<double const &>(it->second);
				++applied;
			} catch (...) {
				try {
					s.*field = static_cast<double>(static_cast<int64_t const &>(it->second));
					++applied;
				} catch (...) {}
			}
		};
		auto set_int = [&](const std::string_view key, int AssStyle::*field) {
			const auto it = fields.find(key);
			if (it == fields.end()) return;
			try {
				s.*field = static_cast<int>(static_cast<int64_t const &>(it->second));
				++applied;
			} catch (...) {}
		};
		auto set_bool = [&](const std::string_view key, bool AssStyle::*field) {
			const auto it = fields.find(key);
			if (it == fields.end()) return;
			try {
				s.*field = static_cast<bool const &>(it->second);
				++applied;
			} catch (...) {}
		};
		auto set_color = [&](const std::string_view key, agi::Color AssStyle::*field) {
			const auto it = fields.find(key);
			if (it == fields.end()) return;
			std::string str;
		...
		for (int i = 0; i < 3; ++i) {
			...
			try {
				s.Margin[i] = static_cast<int>(static_cast<int64_t const &>(it->second));
				++applied;
			} catch (...) {}
		}
	...
	AssStyle &RequireStyle(const agi::Context *ctx, std::string const &name) {
	...
	void RequireStyleNameAvailable(const agi::Context *ctx, std::string const &name) {
	...
	void ValidateStyleFields(Object const &fields) {
		auto check_color = [&](const std::string_view key) {
			const auto it = fields.find(key);
			if (it == fields.end()) return;
			std::string str;
			try { str = static_cast<std::string const &>(it->second); } catch (...) { return; }
			try {
				[[maybe_unused]] agi::Color c(str);
			} catch (...) {
				throw std::runtime_error("invalid color value for '" + std::string(key) + "': " + str);
			}
		};
	...
	/// 对内存数据做 base64 编码
	std::string Base64Encode(void const *data, const size_t len) {
	...
	int64_t ResolveTargetFrame(const agi::Context *ctx, Object const &args) {
		...
		const int total = ctx->project->VideoProvider()->GetFrameCount();
		return std::clamp<int64_t>(frame, 0, total > 0 ? total - 1 : 0);
	}

	/// 解码指定帧并做 HDR 色调映射与黑边填充，返回可保存/编码的 wxImage
	wxImage GetVideoFrameImage(const agi::Context *ctx, const int64_t frame, bool raw) {
		auto *provider = ctx->project->VideoProvider();
		const auto vf = provider->GetFrame(static_cast<int>(frame), ctx->project->Timecodes().TimeAtFrame(static_cast<int>(frame)), raw);
		if (!vf)
	...
	/// 构造一个 integer 类型属性
	UnknownElement MakeIntProp(std::string const &desc, const int64_t def = 0) {
		Object p;
	...
	/// 构造一个 bool 类型属性
	UnknownElement MakeBoolProp(std::string const &desc, const bool def = false) {
		Object p;
	...
	UnknownElement DialogueToJson(AssDialogue const &diag, const int idx) {
		Object obj;
		obj["index"] = idx;
		obj["class"] = std::string(diag.Comment ? "comment" : "dialogue");
		obj["layer"] = diag.Layer;
		...
		obj["margin_l"] = diag.Margin[0];
		obj["margin_r"] = diag.Margin[1];
		obj["margin_v"] = diag.Margin[2];
		return obj;
	}
...
// ---------------------------------------------------------------------------
	...
	std::vector<std::string> FindSimilarCommands(const std::string_view name, const size_t max_count = 3) {
		std::vector<std::pair<size_t, std::string>> scored;
		// 编辑距离阈值：名称越长容错越多，但不超过名称长度的三分之一
		const size_t threshold = std::max<size_t>(3, name.size() / 3);
		for (auto const &cmd_name : cmd::get_registered_commands()) {
			if (size_t dist = agi::util::edit_distance(name, cmd_name); dist <= threshold)
				scored.emplace_back(dist, std::string(cmd_name));
		}
		std::ranges::sort(scored);
		std::vector<std::string> ret;
		for (auto &cmd_name : scored | std::views::values) {
	...
	UnknownElement HandleRunCommand(Object const &args) {
		const auto command = GetString(args, "command");
		if (command.empty())
		...
		try {
			...
			const auto suggestions = FindSimilarCommands(command);
	...
	const std::set<std::string_view, std::less<>> blocking_commands = {
		// 自动化
		"am/manager",
		"am/meta",
		// 应用程序
	...
	const std::map<std::string_view, std::string_view, std::less<>> command_to_tool = {
	...
	UnknownElement HandleListCommands(Object const & /*args*/) {
		...
		for (auto const &name : cmd::get_registered_commands()) {
			...
			if (auto it = command_to_tool.find(name); it != command_to_tool.end())
	...
	UnknownElement HandleGetDialogueLines(Object const &args) {
		const auto *ctx = RequireContext();
		const int64_t limit = GetInt(args, "limit", 0);
		const int64_t offset = GetInt(args, "offset", 0);
		bool include_comment = false;
		if (const auto it = args.find("include_comment"); it != args.end()) {
	...
	UnknownElement HandleUpdateStyle(Object const &args) {
		...
		const auto fit = args.find("fields");
		if (fit == args.end())
			throw std::runtime_error("missing 'fields'");
		Object const *fields = nullptr;
		try {
		...
		const std::string new_name = GetString(*fields, "name");
		...
		const auto undo = GetString(args, "undo_point");
	...
	UnknownElement HandleSearchDialogue(Object const &args) {
		const auto *ctx = RequireContext();
		const auto pattern = GetString(args, "pattern");
		if (pattern.empty())
			throw std::runtime_error("missing 'pattern'");
		bool regex = false;
		if (const auto it = args.find("regex"); it != args.end()) {
		...
		std::regex compiled;
		if (regex) {
			try { compiled = std::regex(pattern); } catch (...) { throw std::runtime_error("invalid regex pattern"); }
		}
	...
	UnknownElement HandleUpdateSubtitleFields(Object const &args) {
		...
		const auto it = args.find("line_index");
		...
		for (auto &diag : ctx->ass->Events) {
			...
			auto set_str = [&](const std::string_view key, boost::flyweight<std::string> AssDialogue::*field) {
				const auto f = fields->find(std::string(key));
				if (f != fields->end()) {
					try { diag.*field = f->second; } catch (...) {}
				}
			};
			[[maybe_unused]] auto set_str_plain = [&](const std::string_view key, std::string AssDialogue::*field) {
				if (const auto f = fields->find(std::string(key)); f != fields->end()) {
			...
			if (auto f = fields->find("layer"); f != fields->end()) {
				try {
					const int64_t v = static_cast<int64_t const &>(f->second);
	...
	UnknownElement HandleInsertSubtitleLine(Object const &args) {
		...
		// 从 fields 参数加载字段
		if (auto fit = args.find("fields"); fit != args.end()) {
			try {
	...
	UnknownElement HandleDeleteSubtitleLine(Object const &args) {
		...
		std::ranges::sort(
			ptrs, [](const AssDialogue *a, const AssDialogue *b) {
				return a->Row > b->Row;
			}
		);
		...
		if (!ctx->ass->Events.empty()) {
			...
			for (auto &diag : ctx->ass->Events) {
				if (ctx->selectionController->GetSelectedSet().contains(&diag))
					new_sel.insert(&diag);
			}
			if (new_sel.empty())
				new_sel.insert(&*ctx->ass->Events.begin());
			if (!new_sel.contains(active))
	...
	UnknownElement HandleShiftTimes(Object const &args) {
		...
		if (const int64_t offset = GetInt(args, "offset", 0); offset != 0) {
			start_offset = offset;
			end_offset = offset;
		}
		...
		Array const *indices = nullptr;
		if (const auto iit = args.find("line_indices"); iit != args.end()) {
			try {
	...
	UnknownElement HandleGetAudioSelection(Object const & /*args*/) {
		...
		const auto range = ctx->audioController->GetPrimaryPlaybackRange();
		Object result;
		result["has_audio"] = !!ctx->project->AudioProvider();
		result["start"] = range.begin();
		result["end"] = range.end();
		result["duration"] = range.length();
		return result;
	}
	...
	UnknownElement HandleFrameFromMs(Object const &args) {
		const auto *ctx = RequireContext();
		const int64_t ms = GetInt(args, "ms", 0);
		...
		Object result;
		result["frame"] = ctx->videoController->FrameAtTime(static_cast<int>(ms), agi::vfr::START);
		result["ms"] = ms;
		return result;
	}
	...
	UnknownElement HandleMsFromFrame(Object const &args) {
		const auto *ctx = RequireContext();
		const int64_t frame = GetInt(args, "frame", 0);
		if (!ctx->project->Timecodes().IsLoaded())
			throw std::runtime_error("no timecodes loaded");
		if (!ctx->videoController)
			throw std::runtime_error("no video controller available");
		...
		Object result;
		result["ms"] = ctx->videoController->TimeAtFrame(static_cast<int>(frame), agi::vfr::START);
		result["frame"] = frame;
		return result;
	}
	...
	UnknownElement HandleVideoSize(Object const & /*args*/) {
		...
		const auto provider = ctx->project->VideoProvider();
		Object result;
		result["has_video"] = true;
		result["width"] = provider->GetWidth();
		result["height"] = provider->GetHeight();
	...
	UnknownElement HandleDecodePath(Object const &args) {
		...
		const auto *ctx = agi::mcp::ActiveContext();
		std::string decoded;
		if (ctx)
	...
	UnknownElement HandleTimeToMs(Object const &args) {
		const std::string time_str = GetString(args, "time");
		if (time_str.empty())
	...
	UnknownElement HandleMsToTime(Object const &args) {
		const int64_t ms = GetInt(args, "ms", -1);
		if (ms < 0)
	...
	UnknownElement HandleClipboardSet(Object const &args) {
		...
		if (!wxTheClipboard->Open())
			throw std::runtime_error("failed to open clipboard");
		const bool ok = wxTheClipboard->SetData(new wxTextDataObject(wxString::FromUTF8(text)));
		wxTheClipboard->Close();
		if (!ok)
	...
	UnknownElement HandleLoadTimecodes(Object const &args) {
		...
		if (path.empty())
			throw std::runtime_error("missing 'path'");
		const agi::fs::path fs_path(wxString::FromUTF8(path).ToStdWstring());
	...
	auto VsmodCurlWriteCb(const char *contents, const size_t size, const size_t nmemb, void *userdata) -> size_t {
	...
	std::string FetchVsmodDocOnline() {
		const auto url = "https://raw.githubusercontent.com/wiki/mojie126/Aegisub/AssRocket-VSFilterMod-%E4%BD%BF%E7%94%A8%E6%96%87%E6%A1%A3.md";
	...
	Object ParseVsmodDoc(std::vector<std::string> const &lines, std::string const &tag) {
		if (!tag.empty()) {
			...
			if (const auto paren = want.find('('); paren != std::string::npos) want.erase(paren);
			if (want.empty())
			...
			for (size_t i = 0; i < lines.size(); ++i) {
				...
				for (auto pos = lines[i].find('\\'); pos != std::string::npos; pos = lines[i].find('\\', pos + 1)) {
					...
					if (auto cand = lines[i].substr(pos + 1, end - pos - 1); cand == want) {
						matched = true;
						break;
					}
				}
	...
	UnknownElement HandleVsmodSyntax(Object const &args) {
		...
		if (source == "local" || source == "auto") {
			const auto doc_path = FindVsmodDocPath();
			if (!doc_path.empty()) {
				std::string content;
				const auto in = agi::io::Open(doc_path);
				content.assign(std::istreambuf_iterator(*in), std::istreambuf_iterator<char>());
	...
	UnknownElement HandleSetScriptResolution(Object const &args) {
		const auto *ctx = RequireContext();
		const int64_t w = GetInt(args, "width", -1);
		const int64_t h = GetInt(args, "height", -1);
	...
	UnknownElement HandleSetSelection(Object const &args) {
		const auto *ctx = RequireContext();
		const auto it = args.find("line_indices");
		if (it == args.end())
		...
		// 活动行：优先取参数，缺省取选中的第一行
		const int64_t active_idx = GetInt(args, "active_line", -1);
		if (active_idx > 0) {
			int n = 0;
			for (auto &diag : ctx->ass->Events) {
				++n;
	...
	UnknownElement HandlePlayAudio(Object const &args) {
		...
		const int64_t start_ms = GetInt(args, "start_ms", -1);
		int64_t end_ms = GetInt(args, "end_ms", -1);
		if (start_ms < 0)
			throw std::runtime_error("missing 'start_ms'");
		const int64_t total_ms = ctx->project->AudioProvider()->GetNumSamples() * 1000 / std::max(1, ctx->project->AudioProvider()->GetSampleRate());
	...
	UnknownElement HandleGetFrame(Object const & /*args*/) {
		...
		if (!ctx->videoController)
			throw std::runtime_error("no video controller available");
		Object result;
		result["frame"] = ctx->videoController->GetFrameN();
		return result;
	}
	...
	UnknownElement HandleOpenFile(Object const &args) {
		...
		const auto lower = boost::to_lower_copy(fs_path.string());
	...
	UnknownElement HandleOpenVideo(Object const &args) {
		...
		result["video_width"] = provider->GetWidth();
		result["video_height"] = provider->GetHeight();
		int sx = 0, sy = 0;
		ctx->ass->GetResolution(sx, sy);
		result["script_width"] = sx;
		result["script_height"] = sy;
	...
	UnknownElement HandleGetVideoFrame(Object const &args) {
		...
		const bool raw = GetBool(args, "raw", false);
		const int64_t frame = ResolveTargetFrame(ctx, args);
		const wxImage img = GetVideoFrameImage(ctx, frame, raw);
		if (!img.IsOk())
		...
		const auto *stream_buf = os.GetOutputStreamBuffer();
		const std::string b64 = Base64Encode(stream_buf->GetBufferStart(), os.GetSize());

		const int64_t ms = ctx->videoController->TimeAtFrame(static_cast<int>(frame), agi::vfr::START);
		Object frame_info;
		...
		result["total_frames"] = ctx->project->VideoProvider()->GetFrameCount();
	...
	UnknownElement HandleSaveVideoFrame(Object const &args) {
		...
		if (const agi::fs::path out_path(wxString::FromUTF8(path).ToStdWstring()); !img.SaveFile(wxString(out_path.wstring()), type))
	...
	UnknownElement HandleExportGif(Object const &args) {
		...
		auto gifski_write_cb = [](const size_t buf_len, const uint8_t *buf, void *user_data) -> int {
	...
	UnknownElement HandleGetAudioWaveform(Object const &args) {
		const auto *ctx = RequireContext();
		const auto *provider = ctx->project->AudioProvider();
		if (!provider)
			throw std::runtime_error("no audio loaded");

		const int64_t total_samples = provider->GetNumSamples();
		const int64_t total_ms = provider->GetSampleRate() > 0 ? total_samples * 1000 / provider->GetSampleRate() : 0;
		const int64_t start_ms = std::clamp<int64_t>(GetInt(args, "start_ms", 0), 0, total_ms);
		int64_t end_ms = GetInt(args, "end_ms", -1);
		if (end_ms < 0) end_ms = total_ms;
		end_ms = std::clamp<int64_t>(end_ms, start_ms, total_ms);
		const int64_t points = std::clamp<int64_t>(GetInt(args, "points", 256), 1, 4096);

		const int64_t start_sample = start_ms * provider->GetSampleRate() / 1000;
		const int64_t end_sample = end_ms * provider->GetSampleRate() / 1000;
		const auto pts = agi::ComputeWaveform(*provider, start_sample, end_sample, static_cast<size_t>(points));

		Array data;
		data.reserve(pts.size());
		for (auto const &p : pts) {
			Object pt;
			pt["peak_min"] = p.peak_min;
			pt["peak_max"] = p.peak_max;
		...
		result["sample_rate"] = provider->GetSampleRate();
		result["start_ms"] = start_ms;
		result["end_ms"] = end_ms;
	...
	UnknownElement HandleGetAudioSpectrum(Object const &args) {
		...
		const int64_t total_samples = provider->GetNumSamples();
		const int64_t total_ms = provider->GetSampleRate() > 0 ? total_samples * 1000 / provider->GetSampleRate() : 0;
		const int64_t ms = std::clamp<int64_t>(GetInt(args, "ms", 0), 0, total_ms);
		int64_t fft_size = GetInt(args, "fft_size", 1024);
		fft_size = std::clamp<int64_t>(fft_size, 256, 32768);

		const int64_t start_sample = ms * provider->GetSampleRate() / 1000;
		const auto powers = agi::ComputeSpectrum(*provider, start_sample, static_cast<size_t>(fft_size));

		Array data;
		data.reserve(powers.size());
		for (const float v : powers)
...
namespace agi::mcp {
	...
	void RegisterMcpTools() {
		...
		{
			Object props;
			props["command"] = MakeStringProp(
				"Aegisub command name, e.g. 'time/shift'. "
