...
, active_line_connection(context->selectionController->AddActiveLineListener(&DialogStyling::OnActiveLineChanged, this))
{
...

	{
		...
		current_line_text = new wxTextCtrl(cur_line_box->GetStaticBox(), -1, _("Current line"), wxDefaultPosition, FromDIP(wxSize(380, 80)), wxTE_MULTILINE | wxTE_READONLY);
		cur_line_box->Add(current_line_text, 1, wxEXPAND | wxALL, half_pad);
		main_sizer->Add(cur_line_box, 0, wxEXPAND | wxALL, pad);
	}

	{
		auto *styles_box = new wxStaticBoxSizer(wxVERTICAL, this, _("Styles available"));
		style_list = new wxListBox(styles_box->GetStaticBox(), -1, wxDefaultPosition, FromDIP(wxSize(180, 220)), to_wx(context->ass->GetStyles()));
		styles_box->Add(style_list, 1, wxEXPAND | wxALL, half_pad);
		bottom_sizer->Add(styles_box, 1, wxEXPAND | wxRIGHT, pad);
	}

	wxSizer *right_sizer = new wxBoxSizer(wxVERTICAL);
	{
		auto *style_text_box = new wxStaticBoxSizer(wxHORIZONTAL, this, _("Set style"));
		style_name = new wxTextCtrl(style_text_box->GetStaticBox(), -1, "", wxDefaultPosition, FromDIP(wxSize(200, -1)), wxTE_PROCESS_ENTER);
		style_text_box->Add(style_name, 1, wxEXPAND | wxALL, half_pad);
		right_sizer->Add(style_text_box, 0, wxEXPAND | wxBOTTOM, pad);
	}

	{
		...
		hotkey_box->Add(hotkey_grid, 0, wxEXPAND | wxALL, half_pad);

		auto_seek = new wxCheckBox(hotkey_box_win, -1, _("&Seek video to line start time"));
		auto_seek->SetValue(true);
		hotkey_box->Add(auto_seek, 0, wxLEFT | wxBOTTOM, half_pad);
		hotkey_box->AddStretchSpacer(1);

		right_sizer->Add(hotkey_box, 1, wxEXPAND | wxBOTTOM, pad);
	}

	{
		...
		actions_box->Add(play_video, 0, wxALL, half_pad);

		actions_box->AddStretchSpacer(1);
		right_sizer->Add(actions_box, 0, wxEXPAND, 0);
	}
	bottom_sizer->Add(right_sizer, 0, wxEXPAND);
	main_sizer->Add(bottom_sizer, 1, wxEXPAND | wxLEFT | wxBOTTOM | wxRIGHT, pad);

	{
		...
		main_sizer->Add(button_sizer, 0, wxEXPAND | wxBOTTOM | wxLEFT | wxRIGHT, pad);
	}
