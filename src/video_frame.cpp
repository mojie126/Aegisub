...
wxImage AddPaddingToImage(const wxImage &img, int padding_top, int padding_bottom) {
	if ((padding_top <= 0 && padding_bottom <= 0) || !img.IsOk()) return img;

	const int src_w = img.GetWidth();
	const int src_h = img.GetHeight();
	const int top = std::max(0, padding_top);
	const int bottom = std::max(0, padding_bottom);
	const int dst_h = src_h + top + bottom;
