...
void FontsCollectorThread(AssFile *subs, agi::fs::path const& destination, FcMode oper, wxEvtHandler *collector) {
	agi::dispatch::Background().Async([subs, destination, oper, collector]{
...
, path(*c->path)
{
	...
	auto *destination_sizer = new wxStaticBoxSizer(wxVERTICAL, this, _("Destination"));
	auto *destination_box = destination_sizer->GetStaticBox();
	...
	auto *log_sizer = new wxStaticBoxSizer(wxVERTICAL, this, _("Log"));
	auto *log_box = log_sizer->GetStaticBox();
	collection_log = new wxStyledTextCtrl(log_box, -1, wxDefaultPosition, this->FromDIP(wxSize(600, 300)));
