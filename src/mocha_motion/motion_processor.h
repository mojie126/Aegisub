...
namespace mocha {
	...
	public:
		...
		void prepare_lines(std::vector<MotionLine> &lines, const ClipTrackOptions *clip_options = nullptr);

		/// 后处理行集合（恢复 \t 占位符、去重、合并相同行等）
		/// 对应 MoonScript postprocLines()
		static void postprocess_lines(std::vector<MotionLine> &lines);

		/// 合并相邻相同行（文本和样式相同时扩展时间范围）
		/// 对应 MoonScript combineIdenticalLines()
		static void combine_identical_lines(std::vector<MotionLine> &lines);

		/// 跨行合并：对来自不同源行的结果进行合并
		/// 对应 MoonScript combineWithLine 的跨行部分
		/// 调用者需确保 lines 已按时间排序
		static void cross_line_combine(std::vector<MotionLine> &lines);
	...
	private:
		...
		[[nodiscard]] std::string get_missing_tags(const std::string &block, const std::map<std::string, double> &properties) const;

		/// 获取缺少的 alpha 标签
		/// 对应 MoonScript getMissingAlphas()
		static std::string get_missing_alphas(const std::string &block,
											const std::map<std::string, double> &properties);

		/// 从 AssStyle 提取属性映射
		static std::map<std::string, double> extract_style_properties(const AssStyle *style);
