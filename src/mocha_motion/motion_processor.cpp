...
void MotionProcessor::prepare_lines(std::vector<MotionLine>& lines) {
	...
	for (auto& line : lines) {
		...
		line.run_callback_on_overrides([&line](const std::string& block, int) {
		...
		if (!has_position) {
			...
			line.run_callback_on_first_override([&line](const std::string& block) {
				char buf[64];
		...
		line.run_callback_on_first_override([this, &line](const std::string& block) {
		...
		line.run_callback_on_overrides([this, &line, style](const std::string& block, int) {
			std::string result = block;
			...
			if (std::regex_search(result, m, reset_re)) {
				...
				if (!missing.empty()) {
					result = std::regex_replace(result, reset_re,
		...
		if (options_.rect_clip || options_.vect_clip) {
			line.run_callback_on_overrides([this, &line](const std::string& block, int) {
				std::string result = block;
	...
	int start_frame)
{
	...
	if (options_.relative) {
		...
		if (ref_frame <= 0) {
			wxLogError("Absolute start frame is out of range (before selection start)");
			ref_frame = 1;
		}
		if (ref_frame > static_cast<int>(main_data.length())) {
			wxLogError("Absolute start frame is out of range (beyond tracking data end)");
			ref_frame = static_cast<int>(main_data.length());
		}
	}
