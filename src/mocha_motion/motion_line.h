...
namespace mocha {
	...
	struct MotionLine {
		...
		/// @param res_x 脚本水平分辨率
		/// @param res_y 脚本垂直分辨率
		void calculate_default_position(int style_align, int style_margin_l, int style_margin_r, int style_margin_t, int res_x, int res_y);
		...
		/// @param res_y 脚本垂直分辨率
		/// @return 是否已有 \pos 标签
		bool extract_metrics(int style_align, int style_margin_l, int style_margin_r, int style_margin_t, int res_x, int res_y);
		...
		/// @param line_dur
		/// @return 还原后的文本
		[[nodiscard]] std::string detokenize_transforms_copy(int shift = 0, int line_dur = 0) const;
		...
		/// 对所有标签块执行回调
		void run_callback_on_overrides(const std::function<std::string(const std::string &, int)> &callback);
