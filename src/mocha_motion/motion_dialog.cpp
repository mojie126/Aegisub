...
struct MotionDialogImpl {
	...
	// 加载时的 writeConf 初始值（用于检测状态变化）
	bool initial_write_conf = true;
...
MotionDialogImpl::MotionDialogImpl(agi::Context* c, int res_x, int res_y)
	: d(c->parent, -1, _("Mocha Motion"), wxDefaultPosition, wxDefaultSize,
	    wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
	, ctx(c)
	, script_res_x(res_x)
	, script_res_y(res_y)
{
	// 设置工具提示不自动消失，鼠标移走时才消失
	...
	auto* data_label = new wxStaticText(&d, wxID_ANY,
		_("Paste AE keyframe data or enter file path (no quotes):"));
	data_text = new wxTextCtrl(&d, wxID_ANY, "", wxDefaultPosition,
	                            wxSize(-1, d.FromDIP(100)), wxTE_MULTILINE);
	data_text->SetToolTip(_("Supports Adobe After Effects keyframe data. "
	...
	chk_scale->SetToolTip(_("Apply scaling data to the selected lines. "
	...
	txt_blur_scale = new wxTextCtrl(&d, wxID_ANY, "1.00",
	                                 wxDefaultPosition, wxSize(d.FromDIP(50), -1),
	                                 0, numericValidator);
	txt_blur_scale->SetToolTip(_("Factor to attenuate (or amplify) blur scale ratio. "
	...
	chk_rc_to_vc->SetToolTip(_("Convert rectangular clip to vector clip before tracking. "
	...
	btn_clip_sep->SetToolTip(_("Open a separate dialog to provide independent tracking data for clips. "
	...
	chk_kill_trans->SetToolTip(_("Generate per-frame subtitle lines with interpolated \\t tags. "
	...
	chk_linear->SetToolTip(_("Use \\move + \\t to create linear transition (single output line), "
	                          "instead of generating per-frame lines."));

	chk_clip_only = new wxCheckBox(&d, wxID_ANY, _("&Clip Only"));
	chk_clip_only->SetToolTip(_("Only apply tracking to clip tags. Ignores position, scale, rotation, "
	...
	spin_start_frame = new wxSpinCtrl(&d, wxID_ANY, "1", wxDefaultPosition,
	...
	chk_preview->SetToolTip(_("Annotate original subtitle to preview tracking effect, "
	                           "then click [Play Current Line] to preview."));

	chk_reverse = new wxCheckBox(&d, wxID_ANY, _("Reverse tracking"));
	chk_reverse->SetToolTip(_("Reverse tracking data order. Use when Mocha tracked from "
	...
	initial_write_conf = result.options.write_conf;
	...
	if (ctx->project->VideoProvider()) {
		...
		if (result.options.relative) {
			...
			if (!selected_set.empty()) {
				int collection_start_frame = INT_MAX;
				int collection_end_frame = 0;
				...
				int relative_frame = current_video_frame - collection_start_frame + 1;
...
void MotionDialogImpl::OnOK(wxCommandEvent&) {
	...
	if (!raw.empty()) {
		if (!result.main_data.best_effort_parse(raw, script_res_x, script_res_y)) {
			wxMessageBox(_("Failed to parse tracking data. Please check the data format or file path."),
	...
	if (!has_main_data && !has_clip) {
		wxMessageBox(_("No tracking data provided."),
		             _("Error"), wxICON_ERROR);
		return;
	}
	...
	// 保存持久化配置（对应 MoonScript: options\updateConfiguration, options\write!）
	// 当 writeConf 勾选时保存所有配置；当 writeConf 从 true 变为 false 时仅保存 writeConf 标志
	// 对应 MoonScript: if config.main.writeConf then updateAll; if old != new then write writeConf only
	if (opts.write_conf) {
		MotionConfig::Save(opts);
		if (result.has_clip_data) {
			MotionConfig::SaveClip(result.clip_options);
		}
	} else if (initial_write_conf != opts.write_conf) {
		// writeConf 状态变化（从 true→false），仅更新 writeConf 标志，保留原有其他配置
		MotionOptions saved_opts;
		MotionConfig::Load(saved_opts);
		saved_opts.write_conf = opts.write_conf;
		MotionConfig::Save(saved_opts);
	}
...
void MotionDialogImpl::OnActivate(wxActivateEvent& event) {
	// 激活时不自动粘贴，避免覆盖用户手动编辑的内容
	// MoonScript 也只在显示对话框前读一次剪贴板
	event.Skip();
}
...
void MotionDialogImpl::OnClipSeparate(wxCommandEvent&) {
	...
	wxDialog clip_dlg(&d, wxID_ANY, _("Clip Tracking Data"),
	...
	auto* data_sizer = new wxStaticBoxSizer(data_box, wxVERTICAL);
	auto* data_label = new wxStaticText(&clip_dlg, wxID_ANY,
		_("Paste clip tracking data or enter file path:"));
	auto* clip_text = new wxTextCtrl(&clip_dlg, wxID_ANY,
		wxString::FromUTF8(clip_data_text_),
		wxDefaultPosition, wxSize(-1, clip_dlg.FromDIP(100)), wxTE_MULTILINE);
	clip_text->SetToolTip(_("Tracking data for clips, independent from main tracking data. "
	...
	auto* spin_csf = new wxSpinCtrl(&clip_dlg, wxID_ANY,
	...
	// 清除数据按钮
	clip_dlg.Bind(wxEVT_BUTTON, [&](wxCommandEvent&) {
		clip_text->Clear();
	...
	if (ret == wxID_OK) {
		...
		if (!clip_data_text_.empty()) {
			...
			if (temp.best_effort_parse(clip_data_text_, script_res_x, script_res_y)) {
				...
				lbl_clip_status->SetLabel(wxString::Format(
