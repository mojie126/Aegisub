...
void MotionHandler::setup_callbacks() {
	...
	if (!options_.clip_only && !main_data_->is_srs()) {
		bool need_pos = options_.x_position || options_.y_position ||
		                options_.x_scale || options_.z_rotation;
                                 ...
                                 std::vector<MotionLine>& result)
{
	...
	// 计算 \t 的起止时间
	int start_frame_abs = frame_from_ms(line.start_time);
	int start_frame_time = ms_from_frame(start_frame_abs);
	int frame_after_start = ms_from_frame(start_frame_abs + 1);
	int frame_before_end_abs = frame_from_ms(line.end_time) - 1;
	int frame_before_end_time = ms_from_frame(frame_before_end_abs);
	int end_frame_time = ms_from_frame(frame_before_end_abs + 1);

	// 从首个字幕帧中点到行起始时间的偏移
	int begin_time = static_cast<int>(std::floor(0.5 * (start_frame_time + frame_after_start))) - line.start_time;
	// 从最后字幕帧中点到行起始时间的偏移
	int end_time = static_cast<int>(std::floor(0.5 * (frame_before_end_time + end_frame_time))) - line.start_time;
	...
	if (line.move.has_value()) {
		...
		double progress = static_cast<double>(line.start_time - move.t1) / (move.t2 - move.t1);
                                    ...
                                    std::vector<MotionLine>& result)
{
	...
	for (int frame = rel_end; frame >= rel_start; --frame) {
		...
		if (options_.kill_trans) {
			...
			// 遍历所有标签块，提取并移除 \fade
			new_text = tag_utils::run_callback_on_overrides(new_text, [&](const std::string& block, int) {
				...
				if (std::regex_search(result_block, m, fade_re)) {
					...
					if (parts.size() == 7) {
						fade = FullFadeData{parts[0], parts[1], parts[2],
                                                  ...
                                                  int begin_time, int end_time)
{
	...
	for (const auto& entry : callbacks_) {
		...
		while (it != end_it) {
			...
			char buf[256];
			std::snprintf(buf, sizeof(buf), "%s%s\\t(%d,%d,%s%s)",
